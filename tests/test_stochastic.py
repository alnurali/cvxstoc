...
class TestStochastic(unittest.TestCase):
    ...
    def test_news_vendor(self):
        ...
        p3 = Problem(Minimize(c*x + 0.5*(-r*y1-s*z1) + 0.5*(-r*y2-s*z2)),
                     [0<=x, x<=b,
                      0 <= y1, 0 <= z1, 0 <= y2, 0 <= z2,
                      y1+z1 <= x, y2+z2 <= x])                              # Putting these constrs in breaks the test:
    ...
    def get_z_data(self, p, p_pos, q):
        ...
        phi = pymc.Container([pymc.CompletedDirichlet(name="phi_" + str(k),
                                                      D=pymc.Dirichlet(name="phi_temp_" + str(k),
                                                                       theta=beta*numpy.ones(N))) for k in range(K)])

        theta = pymc.Container([pymc.CompletedDirichlet(name="theta_" + str(m),
                                                        D=pymc.Dirichlet(name="theta_temp_" + str(m),
                                                                         theta=alpha*numpy.ones(K))) for m in range(M)])

        z = pymc.Container([pymc.Categorical(name="z_" + str(m), p=theta[m], size=N) for m in range(M)])

        w = pymc.Container([pymc.Categorical(name="w_" + str(m) + "_" + str(n),
                                             p=pymc.Lambda("phi_z_" + str(m) + str(n),
        ...
        for m in range(M):
            ...
            rv = WordCountVecRV(model=lda, name="w_0_0", metadata=metadata)         # Note: w_0_0 is just a dummy
    ...
    def test_multistage(self):
        ...
        # Create second stage problem.
        p2 = Problem(Minimize(-s*y1 - r*y2 + expectation(Q2, want_de=True)), [y1+y2<=x, 0<=y1, y1<=d, y2>=0])
        Q1 = partial_optimize(p2, [y1, y2])

        # Create and solve first stage problem.
        p1 = Problem(Minimize(b*x + expectation(Q1, want_de=True)), [0<=x, x<=u])
        p1.solve()
