...
DIDNT_PASS_EITHER_RV_OR_MODEL = "You didn't pass either a pymc random variable or pymc.Model object into the " \
...
CANT_FIND_PYMC_RV_IN_PYMC_MODEL_OBJ = "The pymc random variable name that you passed into the RandomVariable " \
...
BAD_RV_DIMS = "The pymc random variable that is attached to the pymc.Model object that you passed into the " \
...
UNSUPPORTED_PYMC_RV = "It appears that pymc.Model object that you passed into the RandomVariable constructor " \
...
BAD_VAL_MAP = "You passed into RandomVariableFactory a vals list that contains items that are neither int, float, " \
              "nor numpy.ndarray."

UNSUPPORTED_CONSTR = "You created a Problem object that you passed into partial_optimize that contained a constraint " \
                     "that is unsupported right now."

# Used by partial_optimize.py.
CANT_DETERMINE_CURVATURE = "You called partial_optimize with a Problem object that contains neither a Minimize nor a " \
