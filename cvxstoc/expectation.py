...
from partial_optimize import PartialProblem
...
def clamp_or_sample_rvs(expr, rvs2samples={}, want_de=None, want_mf=None, num_samples=None, num_burnin_samples=None, sample_idx=None, sample_idxes=None):
    ...
    while True:
        ...
        if isinstance(cur_expr, RandomVariable):
            ...
            elif isinstance(cur_expr, PartialProblem):
                my_queue.put(ExpectationQueueItem(cur_expr._my_prob.objective._expr))
                
                for constr in cur_expr._my_prob.constraints:
                    my_queue.put(ExpectationQueueItem(constr.lh_exp))
                    my_queue.put(ExpectationQueueItem(constr.rh_exp))
